...
const PANEL_TOGGLE_ANIMATION_MS = 260;
...
export const MachineGlobalPanel = ({
  ...
  const [panelExpandedVisual, setPanelExpandedVisual] = useState(false);
  ...
  const machineErrorCount = machineIssues.filter(
      ...
      {panelVisible && (
        ...
        >
          <div className="overflow-hidden">
            <div className="bg-slate-900/95 backdrop-blur-xl border border-slate-700 rounded-xl shadow-2xl p-4 space-y-4 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar">
              ...
              {activeTab === "general" && (
                <div className="space-y-4 animate-in fade-in">
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            const formatted = formatIdentifier(
                              event.target.value,
                            );
                          ...
                          onBlur={() => {
                            const cleanName =
                              cleanIdentifier(canonicalNameDraft) ||
                              config.canonicalName;
                  ...
                  <div
                    className="pt-2 border-t border-slate-800 relative"
                    ref={dropdownRef}
                  >
                    <label className="block text-[10px] text-slate-400 mb-1 font-mono uppercase tracking-wider">
                      ...
                      <div className="absolute bottom-full mb-0 left-0 w-full bg-slate-800 border border-slate-700 rounded-lg shadow-2xl z-[70] max-h-48 overflow-y-auto py-1">
                        {universes.length === 0 && (
                          <div className="p-3 text-xs text-slate-500 text-center">
                            {t("machine.noUniverses")}
                          </div>
                        )}
                        {universes.map((universe) => {
                          const universeCanonicalId =
                            universe.data.id || universe.data.name;
                          const realities = getRealitiesForUniverse(
                            universe.id,
                          );
                          ...
                          return (
                            ...
                            >
                              <button
                                onClick={() =>
                                  addInitial(`U:${universeCanonicalId}`)
                                }
                                disabled={config.initials.includes(
                                  `U:${universeCanonicalId}`,
                                )}
                              ...
                              >
                                <UniverseIcon
                                  size={12}
                                  className={
                                    STUDIO_ICON_REGISTRY.entity.universe.colors
                                      .base
                                  }
                                />
                              ...
                              {realities.map((reality) => {
                                const realityCanonicalId =
                                  reality.data.id || reality.data.name;
                ...
                <div className="space-y-4 animate-in fade-in">
                  ...
                  <div className="space-y-4">
                    <div className="bg-slate-950/50 p-2.5 rounded-lg border border-slate-800/50">
                      ...
                      >
                        <EntryPhaseIcon size={10} />{" "}
                        {t("machine.constants.entryPhase")}
                      </div>
                    ...
                    <div className="bg-slate-950/50 p-2.5 rounded-lg border border-slate-800/50">
                      ...
                      >
                        <ExitPhaseIcon size={10} />{" "}
                        {t("machine.constants.exitPhase")}
                      </div>
                      <div className="space-y-2">
                        <BehaviorArrayEditor
                    ...
                    <div className="bg-slate-950/50 p-2.5 rounded-lg border border-slate-800/50">
                      ...
                      >
                        <OnTransitionPhaseIcon size={10} />{" "}
                        {t("machine.constants.onTransition")}
                      </div>
                      <div className="space-y-2">
                        <BehaviorArrayEditor
                          items={uc.actionsOnTransition}
                          onChange={(items) =>
                            updateUC("actionsOnTransition", items)
                          }
                          type="action"
                          openBehaviorModal={openBehaviorModal}
                          registry={registry}
                        ...
                        <BehaviorArrayEditor
                          ...
                          onChange={(items) =>
                            updateUC("invokesOnTransition", items)
                          }
                          type="invoke"
                          openBehaviorModal={openBehaviorModal}
                          registry={registry}
